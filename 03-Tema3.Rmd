...
```{r, echo=FALSE,fig.cap= "Árbol de decisión del diseño univariable de medidas repetidas",fig.align='center',out.width="70%"}
knitr::include_graphics("figurasR/anova1fmr3.png")
```
...
tabla<- xtable(datos, caption = "Datos del ejemplo 3.1")
tab.cor<- round(cor(datos),2)
tabla2 <- xtable(tab.cor, caption = "Correlaciones del ejemplo 3.1")
...
knitr:: kable(datos,align = "ccc",caption = "Datos del ejemplo 3.1")
knitr:: kable(tab.cor,align = "ccc",caption = "Correlaciones del ejemplo 3.1")
...
```{r,echo = FALSE,fig.cap= "Boxplot del ejemplo 3.1",,message=FALSE,warning=FALSE,out.widht="80%",fig.align='center'}

#boxplot(datos, col= c('red','green','orange'), ylab= "Respuesta galvánica")

knitr:: include_graphics("figurasR/boxplot31.png")
...
knitr:: kable(rangos,caption = "Rangos del ejemplo 3.1")
```
...
\noindent tenemos que P($\chi^2(2)$ > Q) = `r 1 - round(pchisq(8,2),3)` por lo que rechazamos la hipótesis nula de que los rangos medios de cada condición proceden de la misma población. Utilizando el programa R obtenemos los mismo resultados:
...
- Definimos las variables e introducimos cada variable en su lugar. Asimismo, damos nombre a la variable dependiente:
