...
write.sav(ejemplo1,"ejemplo1.1.sav")
...
lt<-leveneTest(rendimiento~metodo)
knitr:: kable(lt, caption= "Prueba de homogeneidad de varianzas del ejemplo 1.1")
```
...
t.test(rendimiento, mu = 6)
```

Aceptamos la hipótesis de que el valor medio de la variable rendimiento es 6 en la población.
...
```{r, echo=FALSE,fig.cap = "Distribución binomial",warning=FALSE,message=FALSE,fig.align='center'}
...
knitr:: include_graphics("binomial11.png")
...
```{r, echo=FALSE,fig.cap= "Prueba de normalidad Shapiro-Wilk en el programa SPSS",out.width="70%"}
...
\noindent Será necesario introducir la VD a estudiar en el cuadro de "Variable a contrastar". El valor del contraste es cero por defecto, pero habrá que introducir el valor de la hipótesis a estudiar.
