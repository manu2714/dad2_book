...
El contraste compara las diferencias entre medias maestrales con la variabilidad experimental para decidir si ésta ha podido generar esas diferencias o no. Tanto la diferencia entre las medias (MCA) como la variabilidad experimental (varianza intrasujeto ó MCE) son dos formas de estimar la varianza poblacional. Bajo el supuesto de hipótesis nulas, ambas varianzas deben ser iguales.
...
*Ejemplo 1 :* A continuación se presentan los datos de un estudio en el que se quiso comparar los resultados en una prueba de atención de tres grupos de 5 niños (A = sanos, B = con tumor astrocitoma y C = con tumor meduloblastoma)
...
library(xtable)
a=xtable(a,caption="Datos del ejemplo 1")
print(a,comment = F)
```


```{r,echo = FALSE,fig.cap = 'Datos ejemplo 1'}
...
library(xtable)
b=xtable(b,caption="Resultados del ejemplo 1")
print(b,comment = F)
...
Otra forma de plantear este modelo es descomponiendo la puntuación de los sujetos en dos elementos: 1) la variabilidad explicada por el modelo y 2) la variabilidad no explicada por el modelo (error). De este modo, la predicción el modelo (media del grupo) puede descomponerse en dos componentes (media total + diferencia de la media del grupo con respecto a la media total):

$$
Y_{ij} = \mu_{..} + (\mu_j - \mu_{..}) + \epsilon_i
$$

donde $\mu_{..}$ es la media de todas las puntuaciones, $(\mu_j - \mu_{..})$ es la diferencia de la media del grupo y la media total y $\epsilon_i$ es el error. En el caso de la primera puntuación del grupo sano sería:

              30 = `r mean(aciertos)` + `r mean(aciertos[1:5]) - mean(aciertos)`+ `r 30 - mean(aciertos[1:5])` 
        
## 3.1. Comprobación de los supuestos

Es necesario determinar si las muestras son independientes. Esto se asume siempre y cuando hayan sido extraídas al azar [^2]. Asimismo, se necesita determinar sin las muestras proceden de una población normal con la misma varianza. Asimismo, tendremos que comprobar que la distribución de los errores sigue una ley normal. 

[^2]: @ato2 proponen utilizar el test de Durbin-Watson para comprobar la independencia de los residuales del modelo.
...
El tamaño del efecto pretende determinar la importancia del efecto encontrado, ya que es bastante posible encontrar diferencias significativas, que son insignificantes,  cuando los tamaños muestra les son muy grandes. Por tanto, habremos encontrado diferencias significativas en nuestro estudio, pero que resultan irrelevantes a nivel poblacional. Es decir, encontrar diferencias significativas no siempre implica que existan diferencias entre las medias a nivel poblacional.
  
Aunque existen distintas medidas del tamaño del efecto, aquí vamos a considerar la medida de eta cuadrado parcial corregida (calculada por el programa estadístico SPSS) y omega cuadrado que es la medida preferida por distintos investigadores [@pardo].
  
El estadístico eta cuadrado parcial corregida se define como:

$$
\hat{\eta}^2_{corregida} = 1 - [(1 - \hat{\eta}^{2}(N - 1)/(N - J))] 
$$
...
Aplicado a los datos del ejemplo 1 tenemos:
...
Encontramos que sólo existen diferencias significativas entre el grupo A de los individuos sanos y el grupo C de los niños con el tumos de meduloblastoma. Como el nivel de riesgo es igual a 0,005, menor que el criterio de significación de Bonferroni (0.0167) concluimos que existen diferencias significativas entre ambos grupos. Aplicando el criterio de Sidak obtendríamos las mismas conclusiones en este caso: 
...
\noindent donde *q* es el cuantil de la distribución del rango estudentizado que corresponde a un nivel de significación $\alpha_F$ con *J* medias y *N - J* grados  de libertad. Aplicado al ejemplo 1 los resultados serían los siguientes:
...
Cuando la variable factor (VI) del estudio admite la ordenación,se pueden realizar las denominadas comparaciones de tendencia que nos indica el tipo de relación existente entre las variables.
Pueden compararse tantas tendencias como número de niveles de factor menos 1 (*J* - 1). La tendencia más básica es la lineal que supone incrementos (o decrementos) constantes en los distintos valores del factor. Sin embargo, también es posible encontrar tendencias de tipo cuadrático, cúbico, etc. En la siguiente figura se muestran algunas de estas posibilidades.



```{r, echo=FALSE,fig.cap= "Tipos de tendencia",out.width="70%"}
knitr::include_graphics("figurasR/tenden1.png")
```

Para determinar la existencia de una determinada tendencia es necesario realizar una serie de comparaciones entre las medias de los niveles del factor. Estas medias se multiplican por una serie de coeficientes que varían dependiendo de la tendencia queremos contrastar y de los niveles que presenta el factor. Así, por ejemplo, si la VI tiene 3 niveles los coeficientes para las tendencias lineal y cuadrática podrían ser:
...
Este estadístico se distribuye según el modelo de probabilidad F con *J-1* y *gl* grados de libertad. Y *gl* se obtiene mediante:
...
Existen diferentes métodos para realizar las comparaciones a posteriori en el caso de que no se cumpla la homogeneidad de varianzas. Cuando los tamaños muestrales son grandes varios autores recomiendan el método de Games y Howell:
...
```{r, echo=FALSE,fig.cap= "Prueba de Kruskall-Wallis",out.width="70%"}
...
library(xtable)
a=xtable(a,caption="Datos del ejemplo 2")
print(a,comment = F)
```


```{r,echo = FALSE}
plot(aciertos2~grupo, col=c("green","red","orange"),horizontal=T,
main="Figura ejemplo 2" ,xlab="Método", ylab= "Rendimiento")
```
...
dat<- data.frame(b,grupo)
library(xtable)
a=xtable(a,caption="Rangos del ejemplo 2")
print(a,comment = F)
```

En el cuadro 5 aparece el rango para cada una de las puntuaciones. Calculando las medias de los rangos de cada uno de los grupos observamos que las menores puntuaciones están en el grupo C seguido del grupo B:
...
Al igual que en el caso del ANOVA, para conocer cuáles son las diferencias significativas entre los grupo será necesario realizar una prueba *post-hoc*.

### Comparaciones a posteriori

Cuando no se cumplen los supuestos de aplicación del ANOVA las comparaciones a posteriori se realizan mediante la técnica *U de Mann-Whitney* (MW). Esta prueba sirve, al igual que la prueba T de Student para comparar dos muestras independientes. Resulta útil cuando no se cumple la hipótesis de normalidad o los datos son ordinales. 
...
$$
Como la suma de los N rangos vale N(N+1)/2 se verifica que:
...
En la opción *"ANOVA de un factor"* es posible elegir entre la prueba de Fisher que asume homogeneidad de varianzas y la prueba de Welch que no asume igualdad de las varianzas. 
...
Si se elige la opción de *"ANOVA"* sólo estará disponible la posibilidad de comparar grupos con varianzas homogéneas. Sin embargo, esta opción presenta la ventaja de obtener diferentes medidas del tamaño del efecto.
...
\noindent Pulsando el botón de *Opciones* podemos acceder a los descriptivos, a la prueba de homogeneidad de varianzas y a las pruebas de Brown_Forsythe y Welch.
