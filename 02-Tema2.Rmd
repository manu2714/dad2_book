...
```{r,echo = FALSE,fig.cap = 'Boxplot de los resultados del ejemplo 2.1',,out.width="80%",fig.align='center'}
#plot(aciertos~grupo, col=c("green","red","orange"),horizontal=T,xlab="Grupos",ylab= "Aciertos")
knitr:: include_graphics("figurasR/boxplot21.png")
```
...
b=xtable(c,caption="Prueba de Levene del ejemplo 2.1")
print(b,comment = F)
knitr::kable(c, caption= "Prueba de Levene del ejemplo 2.1")
```
...
```{r, echo=FALSE,fig.cap= "Esquema del diseño para la prueba de Kruskall-Wallis",out.width="70%",fig.align='center'}
...
```{r,echo = FALSE, fig.align='center',fig.cap="Boxplot del ejemplo  2.2", out.width="80%"}
# library(ggplot2)
...
knitr::include_graphics("figurasR/boxplot22.png")
```
...
Cuando no se cumplen los supuestos de aplicación del ANOVA las comparaciones a posteriori se pueden realizar mediante la técnica *U de Mann-Whitney* (MW)[^2.3]. Esta prueba sirve, al igual que la prueba T de Student para comparar dos muestras independientes. Resulta útil cuando no se cumple la hipótesis de normalidad o los datos son ordinales. 
